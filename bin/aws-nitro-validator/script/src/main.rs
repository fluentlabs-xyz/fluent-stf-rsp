...
use sp1_sdk::Prover;
use sp1_sdk::{include_elf, Elf, HashableKey, ProverClient, ProvingKey, SP1Stdin};
const ELF: Elf = include_elf!("nitro-validator");
...
async fn main() {
    ...
    let mut proof_groth16 =
        client.prove(&pk, stdin).groth16().await.expect("Failed to generate Groth16 proof");
    ...
    client.verify(&proof_groth16, &vk, None).expect("Groth16 verification failed");
